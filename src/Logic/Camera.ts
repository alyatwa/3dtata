...
export default class Camera
{
    ...
    setAngle()
    {
        ...
        // Items
        this.angle.items = {
            default: new THREE.Vector3(80, - 1.45, 1.15),
        ...
        // Set method
        this.angle.set = (_name: string | number) =>
        {
            const angle = this.angle.items[_name]
            if(typeof angle !== 'undefined')
            {
                gsap.to(this.angle.value, 2, { ...angle, ease: Power1.easeInOut })
            }
        }
        ...
        if(this.debug)
        {
            ...
            //this.debugFolder.add(this.zoom.amplitude , 'amplitude').step(10).min(1).max(90).name('amplitude').listen()
    ...
    setZoom()
    {
        ...
        this.renderer.domElement.addEventListener('touchstart', (_event: { touches: string | any[] }) =>
        {
        ...
        this.renderer.domElement.addEventListener('touchmove', (_event: { touches: string | any[]; preventDefault: () => void }) =>
        {
            if(_event.touches.length === 2)
            {
    ...
    setPan()
    {
        ...
        this.renderer.domElement.addEventListener('touchmove', (_event: { touches: string | any[] }) =>
        {
            if(_event.touches.length === 1)
            {
