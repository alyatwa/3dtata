export default class {
	 ...
	 * On
	 */
	on(_names: any, callback: (_resource?: any, _data?: any) => void) {
		const that = this;
		// Errors
    ...
    // Create namespace if not exist
    if (!(that.callbacks['base'] instanceof Object))
        that.callbacks['base'] = {};

    // Create callback if not exist
    if (!(that.callbacks['base'][name] instanceof Array))
        that.callbacks['base'][name] = [];

    // Add callback
    that.callbacks['base'][name].push(callback);
		return this;
	}
	...
	trigger(_name: any, _args?: any) {
		...
		let name = _name //this.resolveNames(_name);
		...
		//if (name.namespace === "base") {
			...
			for (const namespace in that.callbacks) {
				if (
					that.callbacks['base'] instanceof Object &&
					that.callbacks['base'][name] instanceof Array
				) {
					that.callbacks['base'][name].forEach(function (
						callback: any
					) {
